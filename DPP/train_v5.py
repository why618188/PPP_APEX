...
def dpp_calculate_scores(model, data):
    ...
    for i in tqdm(range(len(data)), desc="Calculating Scores"):
        datapoint = data[i]["conversation"][0]
        system, prompt, solution = datapoint["system"], datapoint["input"], datapoint["output"]
        ...
        score = sum(action_probs) / len(action_probs)
        scores.append(score)
...
def select_top1000(model, data, epoch):
    ...
    selected_data = []
    while True:
        kernel = generate_kernel_matrix(feature_vectors, scores, is_score=True)
        ...
        scores = scores[remaining_indices]
...
if __name__ == '__main__':
    ...
    OUTPUT_DIR = "/home/hywang/projects/d2pruning/DPP/model5"
    adapter_path = "/home/hywang/projects/d2pruning/DPP/model5/epoch_3_model"  # manually modify if resume
